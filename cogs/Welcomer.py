...
class welcomer(commands.Cog):
    ...
    async def on_ready(self):
        print("Habibi, Za Welcomer System Iz Online")
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        bg.text((960, 620), f"Hala Habibi! {member.guild.name}", color="white", font=font_big, align="center")
        bg.text((960, 740), f"{member.name} iz late to za party, he came in at number #{member.guild.member_count}.", color="white", font=font_small, align="center")

        img_File = discord.File(fp=bg.image_bytes, filename=randomized_image)
        await welcome_channel.send(f"You are my brazer now, {member.name}")
